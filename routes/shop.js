...
router.get("/info", async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({ success: false, message: "Failed to retrieve information" });
  }
});
...
router.get("/category/:category", verifyToken, (req, res) => {
  Category.find({ cat_name: req.params.category })
    ...
    .sort({ updatedAt: -1 })
    .then((products) =>
      res.json({ success: true, products: products[0].products })
    )
    .catch((err) =>
      res
        .status(404)
...
router.get("/search/:prod_name", verifyToken, (req, res) => {
  Product.find({ prod_name: { $regex: req.params.prod_name, $options: "i" } })
    ...
    .catch((err) =>
      res
        .status(404)
        .json({ success: false, message: "No products found by this name" })
    );
});
    ...
    Category.findOne({ cat_name: req.body.category })
      ...
      .then((product) => {
        Category.findById(product.category).then((category) => {
          ...
          category
            .save()
            .then((category) => res.json({ success: true, product, category }));
        });
      })
      .catch((err) =>
        res.status(404).json({
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      oldProduct.prod_name = newProductName
        ? newProductName
        : oldProduct.prod_name;
...
router.put("/cart/:userId/:productId", verifyToken, async (req, res) => {
  try {
    ...
    if (
      ...
      const currProductIndex = user.cart.items.findIndex(
        (item) => item.productId.toString() === req.params.productId
      );
      user.cart.items[currProductIndex].quantity += Number(req.body.quantity);
      user.cart.items[currProductIndex].prod_total =
        user.cart.items[currProductIndex].quantity * product.price;
    } else
    ...
    res
      .status(404)
      .json({ success: false, message: "Failed adding product to cart" });
  }
});
...
router.put("/cart/delete/:userId/:productId", verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.params.userId);
    const removeProductIndex = user.cart.items.findIndex(
      (item) => item.productId.toString() === req.params.productId
    );
